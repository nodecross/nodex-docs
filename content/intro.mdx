...
We believe that it is necessary to implement a distributed trust model on web infrastructure in order to scale **digital trust** to the global demand of web system and the Internet.

### Problem

In many parts of the world, the rapid acceleration of digital transformation by COVID-19 has brought to light a number of issues of current web system. If digital transformation is promoted on the current web infrastructure, digital identity will be siled for each product or service, and as a result, the customer experience will become rather complicated. As a simple example, customers need to keep managing passwords for each service, and keep providing personal information for each service over and over again.

In order to build digital trust with their customers, service providers also need to collect and keep managing unnecessary personal information that can not be used for their business. As you know, the security cost and responsibility to protect privacy are increasing year by year.

These problems are difficult to solve in each system or application bacause current web infrastructure do not have sufficient functionality as to whether the data transferred is reliable in human social activity.
