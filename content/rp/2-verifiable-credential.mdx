...
There are two main methods to submit VP to a verifier. The first one is to include the VP in the DID authentication response and submit it to a verifier. See more information in [the section of DID Communication](./3-did-auth). The second one is to storage the VP in SDS and grant access to the VP to a verifier.
